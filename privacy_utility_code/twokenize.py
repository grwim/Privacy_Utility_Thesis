...
def simpleTokenize(text):
    ...
    ## ADDED ##
    ## Remove . , ' " urls
    zippedStr = remove_values_from_list(zippedStr, '.')
    zippedStr = remove_values_from_list(zippedStr, ',')
    zippedStr = remove_values_from_list(zippedStr, '\'')
    zippedStr = remove_values_from_list(zippedStr, '\"')
    zippedStr = remove_values_from_list(zippedStr, '“')
    ...
    zippedStr = remove_values_from_list(zippedStr, '‘')
    zippedStr = remove_values_from_list(zippedStr, '’')
    zippedStr = remove_urls_from_list(zippedStr)
...
def remove_values_from_list(the_list, val):
	return [value for value in the_list if value != val.decode('utf-8')]

def remove_urls_from_list(the_list):
	url_regex = re.compile(url)
	return [value for value in the_list if not url_regex.match(value)]
